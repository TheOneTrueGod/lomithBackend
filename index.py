...
# Debug: Log module import (this will show in Vercel logs if module is being inspected)
import sys
print(f"[DEBUG] index.py module imported. Python version: {sys.version}", file=sys.stderr)
print(f"[DEBUG] Module __name__: {__name__}", file=sys.stderr)
if '__file__' in globals():
    print(f"[DEBUG] Module __file__: {__file__}", file=sys.stderr)

# List what should be exported (helps Vercel know what to look for)
__all__ = ['handler']

# Create handler using closure to cache application (avoids module-level variables)
def _create_handler():
    """Create handler function with cached Django application"""
    _app_cache = None
